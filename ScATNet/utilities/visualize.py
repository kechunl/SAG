...
def compute_case(results_list, scores, verbose=False, savepath=None, mode=None, save=False):
    ...
    if savepath is not None and save:
        ...
        results_summary['auc_micro'] = auc['micro']
        results_summary['auc_macro'] = auc['macro']
        results_summary['auc_class'] = [auc[i] for i in range(num_classes)]
...
def plot_weights(weights, savepath=None, fname='weight_curve'):
    for task in range(weights.shape[0]):
        plt.plot(range(weights.shape[1]), weights[task].mean(axis=1), label='task{}'.format(task))
    plt.legend()
    if savepath is not None:
        plt.savefig('{}/{}.pdf'.format(savepath, fname), dpi=300, bbox_inches='tight')
    plt.close()