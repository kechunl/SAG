...
class MultiScaleAttnDataset(Dataset):
    def __init__(self, opts, datasetfile,
        ...
        print('Found {} images in {} before cleaning.'.format(len(self.image_list), datatype))
        img_list = []
        ...
        print('Found {} images in {} after cleaning.'.format(len(self.image_list), datatype))
    ...
    def __getitem__(self, batch_indexes_tup):
        ...
        if np.isnan(attn_map[self.patch_num[0]]).any():
            # some slices don't have heuristic signals. attn_map will be nan.
        ...
        # tissue mask - attn
        if self.opts['tissue_constraint']:
            with open(join(self.opts['tissue_attn'], im_ind+'.pickle'), 'rb') as handle:
                tissue_attn_map = pickle.load(handle)
            tissue_attn_map = [tissue_attn_map[p] for p in self.patch_num]
        else:
            tissue_attn_map = -1
        ...
        if use_transform:
            ...
            try:
                sample = transform_sample({'image': image}) if transform_sample is not None else sample
            except:
                print(batch_indexes_tup)
                print(img_path)
                exit()

        return sample['image'], label, return_label, img_path, attn_map, tissue_attn_map
...
if __name__ == '__main__':
    ...
    opts = json.load(open('../example_config.json'))
    ...
    for data in train_loader:
        image, label, return_label, img_path, attn_map, tissue_attn_map = data
        pdb.set_trace()
        print(label)