'''
Code copied from https://github.com/binli123/dsmil-wsi/blob/master/deepzoom_tiler.py
'''
...
class DeepZoomImageTiler(object):
    ...
    def __init__(self, dz, basename, savepath, target_levels, mag_base, format, associated, queue):
        self._dz = dz
        ...
        self._tiledir = savepath
        if not os.path.exists(self._tiledir):
            os.makedirs(self._tiledir)
        ...
        for level in range(self._dz.level_count):
            ...
            for row in range(rows):
                for col in range(cols):
                    tilename = os.path.join(self._tiledir, '%d_%d.%s' % (
...
class DeepZoomStaticTiler(object):
    ...
    def __init__(self, slidepath, basename, mag_levels, base_mag, objective, format, tile_size, overlap,
        ...
        img_name = slidepath.split(os.sep)[-1].split('.')[0]
        self._savepath = os.path.join(basename, img_name)
        self._basename = basename
        self._format = format
        self._tile_size = tile_size
        self._overlap = overlap
    ...
    def _run_image(self, associated=None):
        ...
        if os.path.exists(self._savepath.replace('single','single/processed')) or os.path.exists(self._savepath):
            return
        
        tiler = DeepZoomImageTiler(dz, basename, self._savepath, target_levels, MAG_BASE, self._format, associated,
                    self._queue)
        tiler.run()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--datapath', type=str, default='/projects/digipath2/CAMELYON/CAMELYON16/images')
    parser.add_argument('--outpath', type=str, default='/projects/melanoma/Kechun/Camelyon/MIL')
    ...
    parser.add_argument('-v', '--slide_format', type=str, default='tif', help='Image format for tiles [tif]')
    ...
    if len(levels) == 2:
        out_base = os.path.join(args.outpath, 'WSI', args.dataset, 'pyramid')
    else:
        out_base = os.path.join(args.outpath, 'WSI', args.dataset, 'single')
    all_slides = sorted(glob.glob(os.path.join(args.datapath, 'normal_*'+args.slide_format)))[110:]
    
    # pos-i_pos-j -> x, y
    for idx, c_slide in enumerate(all_slides):
        print('Process slide {}/{}'.format(idx+1, len(all_slides)))
        DeepZoomStaticTiler(c_slide, out_base, levels, args.base_mag, args.objective, args.format, args.tile_size, args.overlap, True, args.quality, args.workers, args.background_t).run()
